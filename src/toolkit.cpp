...
Component2D crenels(Vec2 crenel_dims,
    ...
    while (offset < length) {
        ...
        if (real_x <= 0)
            continue;
        crenels_v.push_back(
            Square::create(real_x, crenel_dims.y, false)
...
Component2D crenellated_wall(Vec2 dims,
    ...
    if (right_crenel_params.y > 0) {
        auto right_crenels = crenels(
                ...
                dims.y, right_crenel_params.z);
        right_crenels.rotate(0, 0, -90);
        right_crenels.translate(
                dims.x - right_crenel_params.y, dims.y, 0
        );
